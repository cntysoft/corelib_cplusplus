...
TcpSocketDataDispatchWorker::TcpSocketDataDispatchWorker(const QString &host, quint16 port, ServiceInvoker *invoker)
   : m_host(host),
     m_port(port),
     m_serviceInvoker(invoker)
{
}
...
{
   QMutexLocker locker(&m_serviceInvoker->m_sendBufferMutex);
   m_socket->write(m_serviceInvoker->m_sendBuffer);
   m_socket->flush();
   m_serviceInvoker->m_sendBuffer.clear();
   locker.unlock();
}
...
{
   //监听线程只负责投递数据， 不负责解析数据
   QMutexLocker locker(&m_serviceInvoker->m_receiveBufferMutex);
   QBuffer buffer(&m_serviceInvoker->m_receiveBuffer);
