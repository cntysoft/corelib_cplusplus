...
void WebServiceServer::unboxRequest(const QByteArray &message)
{
   ...
   if(parserError.error == QJsonParseError::NoError){
      if(jsonDoc.isObject()){
         ...
         if(!leakKeys.isEmpty()){
            processProtocolParseError(*socket, SN_E_PROTOCOL_ILL_FORMAT, QString("缺少协议必要字段: %1").arg(leakKeys.join(", ")));
            return;
         }
         ...
         //出错处理
         processProtocolParseError(*socket, SN_E_PROTOCOL_ILL_FORMAT, "协议json根元素不为对象");
      }
   }else{
      //记录日志等等
      //协议错误
      processProtocolParseError(*socket, SN_E_PROTOCOL_ILL_FORMAT, parserError.errorString());
   }
}
